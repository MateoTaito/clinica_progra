...
export class AuthController {
		...
		static editarPaciente = async (req: Request, res: Response) => {
			try {
				...
				if (user_exists) {
					...
					const user = await User.findOne(req.body.rut);
					user.Telefono = req.body.numero;
					user.Nombre = req.body.nombre;
					user.Rut = req.body.rut;
					user.Dirección = req.body.direccion;
	...
	static editarConsulta = async (req: Request, res: Response) => {
		try {
			console.log("Consulta a editar", req.body.rut);
	...
	static deleteAccount = async (req: Request, res: Response) => {
		try {
			...
			if (user_exists) {
				...
				const user = await User.findOne(req.body.rut);
				const consulta = await Consulta.findOne(req.body.rut );
				if (consulta_exists) {await consulta.destroy();}
				await user.destroy();
			}
	...
	static getConsulta = async (req: Request, res: Response) => {
		try {
			console.log("Consulta a buscar", req.body.id);
	...
	static getPaciente = async (req: Request, res: Response) => {
		try {
			console.log("Paciente a consultar", req.body.id);

			const Paciente_exists = await Paciente.findOne({ where: { Rut: req.body.rut} });
